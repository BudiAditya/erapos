...
class Customer extends EntityBase {
	...
	public function LoadAll($entityId = 0,$outletId = 0,$orderBy = "a.kode", $includeDeleted = false) {
	    $sqx = $sqx = "SELECT a.* FROM m_customer AS a Join m_outlet b On a.outlet_id = b.id Where a.id > 0";
	    if ($includeDeleted){
	...
	public function FindById($id) {
		$this->connector->CommandText = "SELECT a.* FROM m_customer AS a WHERE a.id = ?id";
	...
	public function Insert() {
	    $sqx = 'INSERT INTO m_customer (phone,email,outlet_id,kode,nama,alamat,credit_terms,credit_limit,kota,createby_id,create_time)';
	...
	public function Update($id) {
        $sql = 'UPDATE m_customer SET phone = ?phone, email = ?email, outlet_id = ?outlet_id, nama = ?nama, alamat = ?alamat, credit_terms = ?credit_terms, credit_limit = ?credit_limit, kota = ?kota, updateby_id = ?updateby_id, update_time = now() WHERE id = ?id';
	...
	public function Delete($id) {
		$this->connector->CommandText = "UPDATE m_customer SET is_deleted = 1 WHERE id = ?id";
		$this->connector->AddParameter("?id", $id);
		return $this->connector->ExecuteNonQuery();
	}

    public function GetJsonCustomer($ouletId = 0,$orderBy = "a.kode", $includeDeleted = false) {
        $sqx = "SELECT @rownum := @rownum + 1 AS urutan,a.*,c.kode as outlet_kode,c.outlet_name FROM m_customer a Join m_outlet c On a.outlet_id = c.id, (SELECT @rownum := 0) b Where a.is_deleted = 0 And a.outlet_id = $ouletId";
        $this->connector->CommandText = $sqx;
        $rs = $this->connector->ExecuteQuery();
        $data = array();
        if ($rs != null) {
    ...
    public function GetCustomerCode($outletId = 0, $rtype = 1) {
        ...
        $ins = $ckode;
        $this->connector->CommandText = "SELECT a.kode FROM m_customer a WHERE a.outlet_id = $outletId And LEFT(a.kode,4) = ?ins ORDER BY a.kode DESC LIMIT 1";
    ...
    public function GetCustomerCount($ouletId = 0){
        $sql = "Select count(*) as cstCount From m_customer a Where a.is_deleted = 0";
        if ($ouletId > 0){
            $sql.= " And a.outlet_id = ".$ouletId;
        }
        $this->connector->CommandText = $sql;
        $rs = $this->connector->ExecuteQuery();
        $row = $rs->FetchAssoc();
        return strval($row["cstCount"]);
    }
}
